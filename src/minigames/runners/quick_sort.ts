...
export class QuickRunner implements Runner {
    ...
    partition(partitions: Queue<Partition>, model: SSSModel) {
        if (this.l < model.i - 1)
    ...
    medianOfThree(arr: Array<number>, l: number, r: number) {
        const m = Math.floor(l + (r - l) / 2);
        const tmp = [{ idx: l, num: arr[l] }, { idx: m, num: arr[m] }, { idx: r, num: arr[r] }].sort((a, b) => a.num - b.num);
        return tmp[1];
    }
    
    addBatch(model: SSSModel, partition: Partition, checkDone: () => AlgorithmState): void {
        this.addIn(model, checkDone);
    ...
    run(arr: Array<number>): Promise<void> {
        ...
        let pivot: { idx: number, num: number };
